...
Процедура ПриНачалеРаботыСистемы()
	...
	КлиентскоеПриложение.УстановитьЗаголовок("А.Ершов: Бытовые услуги");	
	
	// Получить существующий вид поиска.
	ПланПоиска = ГлобальныйПоиск.ПолучитьПлан();
 	
 	// Удалить из плана поиска поиск в меню "Функции для технического специалиста"
 	// и поиск по справке.
...
Процедура ПриГлобальномПоиске(СтрокаПоиска, ПланПоиска)
	...
	Если СтрНайти(СтрокаПоиска, "№") = 1 Тогда
  		...
  		// Установить собственный вид поиска, 
  		// в котором есть только поиск по номеру документа.
		ПланПоиска.Очистить();
